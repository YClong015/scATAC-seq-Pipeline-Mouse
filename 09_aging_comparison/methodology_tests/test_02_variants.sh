...
# ════════════════════════════════════════════════════════════════════
# 测试 02 的两种改动对 aging DAR 数量的影响：
#   方案 A: 加 sex covariate (design = ~ sex + condition)
#   方案 C: 去掉 CPM top 25% pre-filter
#
# 对 4 个 cell type 跑 4 个 variant：
#   Kidney PC, TAL  (PC=problem, TAL=control)
#   Lung AT2, Mac-alv  (都是 problem)
#
# 4 variants:
#   V0: baseline (当前 02 设置)
#   V1: +sex covariate          (方案 A)
#   V2: -CPM top 25%            (方案 C)
#   V3: +sex AND -CPM           (A + C)
#
# 输出：CSV 表格对比 4 个 variant 的 DAR 数
# ════════════════════════════════════════════════════════════════════
...
def pseudobulk(adata, science_labels, use_cpm_filter=True):
    ...
    for samp in ct.obs['Sample'].unique():
        ...
        # 从 Sample 名字提取 sex (e.g. "Adult_Female_3" -> "Female")
...
print(f"\n{'='*70}")
print(f"  汇总对比表 (saved to {OUT_CSV})")
print(f"{'='*70}\n")
print(df.to_string(index=False))

# 计算每个 cell type 相对 baseline 的提升倍数
print(f"\n{'='*70}")
print(f"  相对 V0_baseline 的提升倍数")
print(f"{'='*70}\n")
