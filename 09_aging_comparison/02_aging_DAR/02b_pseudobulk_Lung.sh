...
#SBATCH --mem=128G
#SBATCH --job-name=pseudobulk_Lung
...
MIN_SAMPLES          = 4
MIN_PEAK_FRAC        = 0.05   # non-zero in >=5% of samples
MIN_MEAN_COUNT       = 1      # mean count >=1 across samples (removes near-zero peaks
                              # to reduce multiple testing burden without losing real signal)
...
# ════════════════════════════════════════════════════════════
# Fig 13: Aging DAR counts per cell type (run after both jobs done)
# ════════════════════════════════════════════════════════════
module load r/4.4.2
...
make_panel <- function(df, tis, panel_label) {
  ...
  # Remove cell types with 0 DARs in both directions
  totals <- df |>
    group_by(cell_type) |>
