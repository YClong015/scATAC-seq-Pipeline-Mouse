...
| Tissue-specific disease closing | Loss of cell identity | HNF1b/HNF4a/PPARa (kidney PT); Smad2/HIF-1a (lung AT2); KLF4/WT1 (aorta SMC); TCF7/LEF1 (T cells) |
...
![Pipeline overview](09_figures/Pipeline_overview.png)
...
```bash
# 1. Clone repo
git clone https://github.com/YClong015/scATAC-ageing-disease-mouse.git
cd scATAC-ageing-disease-mouse
...
sbatch 00_download/download_cellranger_ref.slurm           # 10x Cell Ranger ref for Kidney/Aorta
sbatch 00_download/mkref_MGI.slurm                          # dnbc4tools reference for Lung
sbatch 00_download/download_kidney_sra.slurm
sbatch 00_download/download_aorta_sra.slurm
sbatch 00_download/lung_cngb/download_full_directory.slurm  # MGI / CNGB
sbatch 00_download/download_science_aging.slurm             # prints GEO h5ad download instructions
...
01_preprocessing/        - Per-tissue alignment outs/ -> Seurat (QC, Harmony, annotation)
   ...
   -- lung/atac_Lung.R  +  dnbc4tools_per_sample/Run_MGI_75..80.slurm
   -- aorta/Aortic_scATAC.R
   -- tcells/Tcell_scATAC.R -> Tcell_clean_recluster.R -> Tcell_final_annotate.R   (entry: Ralph's pre-processed RData)
...
05_universal_assays/     - Re-quantify each tissue's Seurat obj against universal peaks
                            (Lung gets an extra prune step via prune_lung_features.R)
...
09_figures/              - All thesis figures + supplementary
environment/             - R + Python + HPC module specs
```

Note: the 4-tissue Harmony integration step (used only to make the integrated UMAP figure for Aim 1) is not part of the canonical DAR pipeline. The script that produced the integrated UMAP lives under `09_figures/Fig5_UMAP/Fig5_integration_UMAP.R`.

---

## Reproducibility notes

- All randomised steps use `set.seed(1234)` (R) or `random_state=1234` (Python).
- Software versions: R 4.4.2, Python 3.10.12, Seurat 5.0.1, Signac 1.10.0, DESeq2 1.40.2, pycisTopic 1.0.3, HOMER 4.11.1, MACS2 2.2.7.1, bedtools 2.30.0, Cell Ranger ATAC 2.0.0, dnbc4tools 2.1.3 (Lung MGI alignment).
- All paths in scripts were developed on UQ Bunya HPC under user `s4869245`. Before running, update them to your own cluster. Besides the `/QRISdata/Q8448/...` data root, the following are author-specific tool/reference installs and must be changed: HOMER (`/scratch/user/s4869245/homer`), the conda env (`/home/s4869245/.conda/envs/...`), the mm10 reference (`/scratch/user/s4869245/mouse_mm10_ref`), and dnbc4tools (`/scratch/user/s4869245/dnbc4tools/...`). Search each script for `/QRISdata/`, `/scratch/user/`, and `/home/` to find every path to edit.
- The peak-calling, peak-merging and universal-peak SLURM scripts call a co-located `.py` via `$SLURM_SUBMIT_DIR`, so submit them from their own directory (e.g. `cd 02_peak_calling/kidney && sbatch peak_kidney.slurm`).
