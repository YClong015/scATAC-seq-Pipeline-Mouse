...
# Run link_fragments_and_save.R first if ready objects don't exist.
# ============================================================
...
ready_dir <- "/QRISdata/Q8448/Mouse_disease_data/CoveragePlot_ready"
out_dir   <- "/QRISdata/Q8448/Mouse_disease_data/QC_figures/CoveragePlots"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

tissue_configs <- list(
  Tcells = list(
    rds       = file.path(ready_dir, "Tcells_ready.rds"),
    label_col = "cell_type",
    gene      = "Cd3e"
  ),
  Aorta = list(
    rds       = file.path(ready_dir, "Aorta_ready.rds"),
    label_col = "cell_type",
    gene      = "Acta2"
  ),
  Lung = list(
    rds       = file.path(ready_dir, "Lung_ready.rds"),
    label_col = "cell_type",
    gene      = "Sftpc"
  ),
  Kidney = list(
    rds       = file.path(ready_dir, "Kidney_ready.rds"),
    label_col = "cell_type",
    gene      = "Epcam"
  )
)

for (tis in names(tissue_configs)) {
  cfg <- tissue_configs[[tis]]

  if (!file.exists(cfg$rds)) {
    message(tis, ": ready object not found. Run link_fragments_and_save.R first.")
    next
  }
  ...
  if (tis == "Tcells") {
    ...
    obj$cell_type <- unname(tcell_annotations[as.character(obj$seurat_clusters)])
  }
  ...
  set.seed(42)
  cells_keep <- unlist(lapply(levels(Idents(obj)), function(ct) {
    ct_cells <- WhichCells(obj, idents = ct)
    if (length(ct_cells) > 200) sample(ct_cells, 200) else ct_cells
  }))
  ...
  if (!is.null(p)) {
    pdf(file.path(out_dir, paste0("CoveragePlot_", tis, "_", cfg$gene, ".pdf")),
        width = 10, height = 7)
    print(p)
    dev.off()

    png(file.path(out_dir, paste0("CoveragePlot_", tis, "_", cfg$gene, ".png")),
