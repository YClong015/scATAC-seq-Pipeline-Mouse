...
# ============================================================
# Lung HOMER motif plots
#
# Input directory structure:
#   HOMER/
#     <CellType>__Case_vs_Control__UP/knownResults.txt
#     <CellType>__Case_vs_Control__DOWN/knownResults.txt
#
# Logic:
#   UP   -> opening-like summary, top 10 across cell types
#   DOWN -> closing-like, top 25 for individual cell types
# ============================================================
...
write.csv(
  df,
  file.path(plot_dir, "HOMER_knownResults_parsed_lung.csv"),
  row.names = FALSE
)

# Keep only expected contrast
df <- df %>%
  filter(contrast == "Case_vs_Control")

# Cell type order from directory names
celltype_levels <- c(
  "AT2", "B", "Ciliated", "EC-vasc", "Eosinophils",
...
# -----------------------------
build_up_plot_data <- function(df_in,
                               top_n_each = 10,
                               top_n_final = 10,
                               q_cut = 0.05) {
  sub_df <- df_in %>%
...
plot_up <- function(plot_df) {
  ggplot(
    ...
    labs(
      title = "Lung UP DAR motifs (top 10 across cell types)",
      x = "Cell type",
      y = "Motif"
    ) +
...
# -----------------------------
build_down_plot_data <- function(df_in,
                                 top_n_each = 25,
                                 q_cut = 0.05) {
  sub_df <- df_in %>%
...
plot_down_facet <- function(plot_df) {
  ggplot(
    plot_df,
    aes(
      x = cell_type,
...
plot_one_down_celltype <- function(plot_df, cell_use) {
  sub_df <- plot_df %>%
...
down_plot <- build_down_plot_data(
  df_in = df,
  top_n_each = 25,
  q_cut = 0.05
)
...
write.csv(
  up_plot,
  file.path(plot_dir, "Lung_up_plot_data_top10.csv"),
  row.names = FALSE
)

write.csv(
  down_plot,
  file.path(plot_dir, "Lung_down_plot_data_top25.csv"),
  row.names = FALSE
)

p_up <- plot_up(up_plot)
p_down_facet <- plot_down_facet(down_plot)
...
# -----------------------------
ggsave(
  file.path(plot_dir, "Lung_HOMER_up_top10.pdf"),
  p_up,
  width = 12,
  height = 7
)
...
ggsave(
  file.path(plot_dir, "Lung_HOMER_up_top10.png"),
  p_up,
  width = 12,
  height = 7,
  dpi = 300
...
# -----------------------------
ggsave(
  file.path(plot_dir, "Lung_HOMER_down_top25_facet.pdf"),
  p_down_facet,
  width = 16,
  height = 16
)
...
ggsave(
  file.path(plot_dir, "Lung_HOMER_down_top25_facet.png"),
  p_down_facet,
  width = 16,
  height = 16,
  dpi = 300
...
# -----------------------------
# 9) Save DOWN per cell type
# -----------------------------
cell_types_down <- unique(as.character(down_plot$cell_type))

for (ct in cell_types_down) {
  p_ct <- plot_one_down_celltype(down_plot, ct)
  
  ggsave(
    file.path(
      plot_dir,
      paste0("Lung_HOMER_down_", gsub(" ", "_", ct), "_top25.pdf")
    ),
    p_ct,
    width = 7,
    height = 8
  ...
  ggsave(
    file.path(
      plot_dir,
      paste0("Lung_HOMER_down_", gsub(" ", "_", ct), "_top25.png")
    ),
    p_ct,
    width = 7,
    height = 8,
