#!/usr/bin/env Rscript
# ============================================================
# Fig. 5 & 6 — Universal peak set (v2, bedtools merge)
...
# Paths
# ---------------------------------------------------------------
upset_csv <- "/QRISdata/Q8448/Mouse_disease_data/universal_peaks_v2/upset_input.csv"
...
# Load data
# ---------------------------------------------------------------
df <- read.csv(upset_csv, stringsAsFactors = FALSE)
...
# ---------------------------------------------------------------
# Per-tissue peak counts (for Fig. 5 bar chart)
# ---------------------------------------------------------------
tissue_counts <- data.frame(
...
tissue_counts$Tissue <- factor(
  tissue_counts$Tissue,
  levels = c("Kidney", "Lung", "Aorta", "T cells", "Universal")
)
tissue_counts$Type <- ifelse(tissue_counts$Tissue == "Universal",
                             "Universal", "Tissue")
...
ggsave(file.path(out_dir, "Fig5A_peak_counts_v2.pdf"), p5,
       width = 7, height = 5)
ggsave(file.path(out_dir, "Fig5A_peak_counts_v2.png"), p5,
       width = 7, height = 5, dpi = 300)
message("Saved: Fig5A_peak_counts_v2")

# ---------------------------------------------------------------
# Fig. 6 — UpSet plot (ComplexUpset)
# ---------------------------------------------------------------
...
p6 <- upset(
  df,
  intersect    = tissues,
  labeller     = labeller(intersection = tissue_labels),
  width_ratio  = 0.2,
...
ggsave(file.path(out_dir, "Fig6_upset_v2.pdf"), p6,
       width = 12, height = 6)
ggsave(file.path(out_dir, "Fig6_upset_v2.png"), p6,
       width = 12, height = 6, dpi = 300)
message("Saved: Fig6_upset_v2")
