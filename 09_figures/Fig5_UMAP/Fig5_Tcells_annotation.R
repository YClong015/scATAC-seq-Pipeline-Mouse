...
if (length(protein.cols) > 0) {
  ...
  cluster.means <- md %>%
    group_by(cell_type) %>%
    summarise(across(all_of(protein.cols), mean, na.rm = TRUE),
              n_cells = n(), .groups = "drop")

  write.csv(cluster.means,
            file.path(OUT_DIR, "Tcells_celltype_protein_means.csv"),
            row.names = FALSE)

  mat <- cluster.means %>%
    select(cell_type, all_of(protein.cols)) %>%
  ...
  p.heat <- ggplot(mat, aes(x = marker, y = cell_type, fill = scaled)) +
...
umap.df <- as.data.frame(emb)
colnames(umap.df) <- c("UMAP_1", "UMAP_2")
umap.df$label     <- as.character(obj@meta.data$cell_type)
umap.df$condition <- as.character(obj@meta.data$deMultliplex2_final_mapped)
umap.df$Tissue    <- "Tcells"

write.csv(umap.df, file.path(CACHE_DIR, "Tcells_umap.csv"))
message("Saved: Tcells_umap.csv")

rm(obj); gc()

## UMAP plot - coloured by cell type
p.umap <- ggplot(umap.df, aes(x = UMAP_1, y = UMAP_2, color = label)) +
...
ggsave(file.path(OUT_DIR, "Fig5_Tcells_UMAP.pdf"),  p.umap, width = 5, height = 4)
ggsave(file.path(OUT_DIR, "Fig5_Tcells_UMAP.png"),  p.umap, width = 5, height = 4, dpi = 300)
message("Saved: Fig5_Tcells_UMAP")

## UMAP plot - coloured by HTO condition (for QC check)
p.cond <- ggplot(umap.df, aes(x = UMAP_1, y = UMAP_2, color = condition)) +
  geom_point(size = 0.3, alpha = 0.6) +
  guides(color = guide_legend(override.aes = list(size = 3), ncol = 1)) +
...
ggsave(file.path(OUT_DIR, "Fig5_Tcells_UMAP_by_condition.pdf"), p.cond, width = 5, height = 4)
ggsave(file.path(OUT_DIR, "Fig5_Tcells_UMAP_by_condition.png"), p.cond, width = 5, height = 4, dpi = 300)
message("Saved: Fig5_Tcells_UMAP_by_condition (QC check)")
