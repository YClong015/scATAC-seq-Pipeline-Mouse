...
#SBATCH --mem=32G
#SBATCH --job-name=overlap_Kidney_oldobj_5movs21mo
...
AGING_DIR="${BASE}/DAR_science_comparison/aging_DARs_5movs21mo"
TMP_DIR="${BASE}/DAR_science_comparison/tmp_beds_Kidney_oldobj_5movs21mo"
OUT_DIR="${BASE}/DAR_science_comparison/results_Kidney_oldobj_5movs21mo"
...
for aging_tsv in "${AGING_DIR}"/*_Aged_vs_Adult_DAR.tsv; do
    base=$(basename "${aging_tsv}" _Aged_vs_Adult_DAR.tsv)
    tissue="${base%%_*}"
    ...
    # Filter: padj<0.05 only (no LFC threshold, matching Lu 2026 / Patrick 2024 aging atlases)
...
# ── Special override: replace PT opening with v5 PCT+PST (padj<0.05 only, no LFC filter) ──
echo "$(date)  Overriding PT opening with v5 PCT+PST (padj<0.05, no LFC filter)..."
...
echo "  PT opening (v5 PCT+PST, padj<0.05 only): ${n_pt_open} peaks"
...
TMP_DIR   = "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/tmp_beds_Kidney_oldobj_5movs21mo"
OUT_DIR   = "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Kidney_oldobj_5movs21mo"
...
for f in sorted(os.listdir(TMP_DIR)):
    ...
    aging_tsv = f"{AGING_DIR}/{base}_Aged_vs_Adult_DAR.tsv"
...
p_bar <- ggplot(plot_df, aes(x = label, y = fold_plot, fill = direction)) +
  ...
  labs(
    title    = "Kidney — Disease DAR overlap with PURE aging DARs (Aged 21mo vs Adult 5mo)",
...
for aging_tsv in "${AGING_DIR}"/*_Aged_vs_Adult_DAR.tsv; do
    ...
    awk -F'\t' 'NR>1 && $7~/^[0-9eE.+-]/ && $7+0 < 0.05 && $3+0 != 0 {
...
PAIRED_DIR <- "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Kidney_oldobj_5movs21mo/paired_lfc"
OUT_DIR    <- "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Kidney_oldobj_5movs21mo"
...
for (f in sort(files)) {
  ...
  dat <- dat %>%
    mutate(
      ...
      # Factor order: Not sig FIRST so it plots underneath grey background;
      # Sig LAST so it plots on top.
      aging_sig = factor(aging_sig, levels = c("Not sig in aging",
                                               "Aging trending (padj<0.2)",
                                               "Aging sig (padj<0.05)"))
    ) %>%
    arrange(aging_sig)   # ensures grey is drawn first, sig last (on top)

  r_all   <- cor(dat$d_lfc, dat$a_lfc, method = "pearson")
  aging_s <- filter(dat, aging_sig == "Aging sig (padj<0.05)")
  # Relaxed set: includes both sig AND trending — used for regression line
  # when sig alone is too small (e.g. PC at 5mo→21mo)
  ...
  p <- ggplot(dat, aes(x = d_lfc, y = a_lfc, color = aging_sig,
                       alpha = aging_sig)) +
    geom_point(size = 0.9) +
    # Regression line: prefer strict (aging sig only); if too few, use
    # relaxed (sig + trending) so weak cell types like PC still get a line.
    { if (nrow(aging_s) >= 5)
...
combined <- wrap_plots(plots, ncol = 3) +
  plot_annotation(
    title    = "Kidney — Disease-significant DARs vs PURE aging accessibility (Aged 21mo vs Adult 5mo)",
...
pA <- ggplot(fisher_long, aes(x = label, y = OR_plot, fill = direction)) +
  ...
  labs(title = "A  Enrichment of disease DARs in PURE aging DARs (Kidney, 5mo→21mo)",
...
r_long <- r_df %>%
  ...
  select(label, tissue, r_all, n_all)

y_lim_low  <- min(-0.7, floor(min(r_long$r_all, na.rm = TRUE)*10)/10 - 0.05)
y_lim_high <- max( 0.7, ceiling(max(r_long$r_all, na.rm = TRUE)*10)/10 + 0.05)

pB <- ggplot(r_long, aes(x = label, y = r_all,
                         fill = ifelse(r_all >= 0, "pos", "neg"))) +
  geom_col(width = 0.55, show.legend = FALSE) +
  geom_text(aes(label = sprintf("n=%d", n_all),
                y = ifelse(r_all >= 0,
                           r_all + 0.025,
                           r_all - 0.025),
                vjust = ifelse(r_all >= 0, 0, 1)),
            size = 3, color = "grey30") +
  ...
  labs(title = "B  Pearson r — disease vs PURE aging (5mo→21mo) log2FC (Kidney)",
       x = NULL, y = "Pearson r (all disease-sig peaks)") +
  theme_bw(base_size = 11) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
        plot.title  = element_text(face = "bold", size = 11))
...
combined <- pA / pB +
  plot_annotation(
    title    = "Kidney: Overlap between CKD DARs and PURE aging DARs (5mo→21mo)",
