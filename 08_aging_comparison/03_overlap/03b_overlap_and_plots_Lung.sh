...
TMP_DIR="${BASE}/DAR_science_comparison/tmp_beds_Lung"
OUT_DIR="${BASE}/DAR_science_comparison/results_Lung"
mkdir -p "${TMP_DIR}" "${OUT_DIR}/figures"

CONTRAST="Case_vs_Control"
DAR_TABLES="${BASE}/DAR_pseudobulk_Lung_DESeq2/DAR_tables"
...
TMP_DIR   = "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/tmp_beds_Lung"
OUT_DIR   = "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Lung"
...
p_bar <- ggplot(plot_df, aes(x = label, y = fold_plot, fill = direction)) +
  ...
  labs(
    title    = "Lung — Disease DAR overlap with aging DARs",
...
PAIRED_DIR <- "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Lung/paired_lfc"
OUT_DIR    <- "/QRISdata/Q8448/Mouse_disease_data/DAR/DAR_science_comparison/results_Lung"
...
combined <- wrap_plots(plots, ncol = 2) +
  plot_annotation(
    title    = "Lung — Disease-significant DARs vs Aging accessibility landscape",
...
pA <- ggplot(fisher_long, aes(x = label, y = OR_plot, fill = direction)) +
  ...
  labs(title = "A  Enrichment of disease DARs in aging DARs (Lung)",
...
if (!is.null(r_df) && nrow(r_df) > 0) {
  ...
  pB <- ggplot(r_long, aes(x = label, y = r_aging_sig)) +
    ...
    labs(title = "B  Correlation of log2FC between disease and aging DARs (Lung)",
